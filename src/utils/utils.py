...
from src.utils.schema import ASSISTANT, DEFAULT_SYSTEM_MESSAGE, FUNCTION, SYSTEM, USER, ContentItem, Message
    
def logger(level=None):
    if level is None:
        if os.getenv('AGENT_DEBUG', '0').strip().lower() in ('1', 'true'):
            level = logging.DEBUG
        else:
            level = logging.INFO
    ...
    try:
        ...
        if not os.path.exists(log_dir):
            print(f"警告: 日志目录 {log_dir} 创建失败")
        elif not os.access(log_dir, os.W_OK):
            print(f"警告: 日志目录 {log_dir} 不可写")
    ...
    try:
        ...
        print(f"成功创建文件处理器，日志将写入: {log_file}")
    except PermissionError:
        print(f"权限错误: 无法写入日志文件 {log_file}，请检查文件/目录权限")
    except FileNotFoundError:
        print(f"文件路径错误: 无法找到路径 {log_file}")
    except Exception as e:
        print(f"创建日志文件处理器时出错: {e.__class__.__name__}: {e}")
